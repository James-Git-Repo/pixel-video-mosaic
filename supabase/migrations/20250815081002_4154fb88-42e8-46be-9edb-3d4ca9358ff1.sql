-- Fix Security Definer View and related security issues
-- This migration addresses multiple security linter warnings

-- 1. Drop any existing security definer views if they exist
-- (The linter detects them but they may not show up in our queries)
DROP VIEW IF EXISTS admin_video_submissions CASCADE;

-- 2. Update all functions to have proper search_path to fix security warnings
-- Update create_slot_hold_atomic function
DROP FUNCTION IF EXISTS public.create_slot_hold_atomic(uuid, text, text, text[], integer);
...
-- Update free_occupied_slots function
DROP FUNCTION IF EXISTS public.free_occupied_slots(uuid);
...
-- 3. Ensure no security definer objects remain
-- Check and drop any remaining security definer views or functions that might be causing the linter warning