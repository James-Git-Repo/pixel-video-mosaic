-- Fix create_slot_hold_atomic function - remove reference to non-existent admin_video_items table
-- The function should only check occupied_slot_items for conflicts, not admin_video_items

CREATE OR REPLACE FUNCTION public.create_slot_hold_atomic(
  p_user_id TEXT,  -- Changed from UUID to TEXT to support anonymous user IDs
  p_top_left TEXT,
  p_bottom_right TEXT,
  p_slot_ids TEXT[],
...
BEGIN
  ...
  -- Only check occupied_slot_items and active slot_holds (removed admin_video_items)
  ...
  -- Create the hold (user_id is now TEXT for anonymous users)
